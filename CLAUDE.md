...
- **Forces have two implementations** (`potential.py`), selected by `use_autograd`: the hand-derived analytical gradient (default, faster) and `torch.autograd.grad` (reference). They exist to cross-check each other — keep them numerically consistent. The hot path `_calculate_force` is a standalone `@torch.compile(dynamic=True)` function (first call compiles, ~6 s); `_calculate_potential` (energy) is left **eager** so second derivatives — e.g. the Hessian via autograd — keep working, since `torch.compile` doesn't support double-backward. Potential parameters are registered buffers, so device/dtype move with the module.
